...
let jsonmodalData: any = {};
  ...
  const [내용5, set내용5] = useState(jsonmodalData.내용5 ||'');
   ...
   const insertButtonclick = async () => {
    ...
    axios.post('/api/googlesheet', inputData)
    .then((response) => {
  ...
  const updateButtonclick = async () => {
    ...
    axios.put('/api/googlesheet', 
    {inputData, jsonmodalData})
    .then((response) => {
      if(response.status === 200){
        alert('수정 완료');
        onClose();
  ...
  // 삭제 버튼 클릭 이벤트 함수
    const deleteButtonclick = async () => {
        ...
        axios.delete('/api/googlesheet', { data: inputData })
        .then((response) => {
        if(response.status === 200){
            alert('삭제 완료');
            onClose();
  ...
  <div className={styles.modalStyle}>
    <div style={{ display: 'flex', alignItems: 'center' }}>
      <div>이름 : </div>
      <input type="text" value={이름} onChange={(e) => set이름(e.target.value)}/>
    </div>
    <div style={{ display: 'flex', alignItems: 'center' }}>
      <div>내용1 : </div>
    <input type="text" value={내용1} onChange={(e) => set내용1((e.target as HTMLInputElement).value)}/>
    </div>
    <div style={{ display: 'flex', alignItems: 'center' }}>
      <div>내용2 : </div>
      <input type="text" value={내용2} onChange={(e) => set내용2(e.target.value)} />
    </div>
    <div style={{ display: 'flex', alignItems: 'center' }}>
      <div>내용3 : </div>
      <input type="text" value={내용3} onChange={(e) => set내용3(e.target.value)} />
    </div>
    <div style={{ display: 'flex', alignItems: 'center' }}>
      <div>내용4 : </div>
      <input type="text" value={내용4} onChange={(e) => set내용4(e.target.value)} />
    </div>
    <div style={{ display: 'flex', alignItems: 'center' }}>
      <div>내용5 : </div>
      <input type="text" value={내용5} onChange={(e) => set내용5(e.target.value)} />
    </div>
